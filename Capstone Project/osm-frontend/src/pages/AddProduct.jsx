...
function AddProduct() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await API.post("/products", { ...product, price: Number(product.price), stockQuantity: Number(product.stockQuantity) });
      ...
      <div className="card max-w-lg">
        <div className="space-y-4">
          <div>
            ...
            <input className="input" placeholder="e.g. Wireless Keyboard" onChange={e => setProduct({ ...product, name: e.target.value })} />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">Category *</label>
            <input className="input" placeholder="e.g. Electronics" onChange={e => setProduct({ ...product, category: e.target.value })} />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleSubmit} className="btn flex-1">Add Product</button>
            <button onClick={() => navigate("/products")} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-lg hover:bg-gray-50 transition">Cancel</button>
          </div>
        </div>
      </div>
    </Layout>
