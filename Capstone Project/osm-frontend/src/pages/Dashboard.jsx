    ...
    const fetchStats = async () => {
      try {
        ...
        if (role === "ADMIN") {
          ...
          setStats({ orders: orders.data.length, products: products.data.length, customers: users.data.length });
        } else {
          const orders = await API.get(`/orders/customer/${user.id}`);
          setStats({ orders: orders.data.length, products: products.data.length, customers: 0 });
        }
      } catch {}
    };
    fetchStats();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {cards.map((c, i) => (
          <div key={i} className="card flex items-center gap-5 hover:shadow-lg transition-shadow">
            <div className={`${c.color} text-white p-4 rounded-2xl shadow-md`}>
              {c.icon}
            </div>
            <div>
