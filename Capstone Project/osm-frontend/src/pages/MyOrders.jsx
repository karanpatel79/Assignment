...
function MyOrders() {
  ...
  return (
    <Layout>
      ...
      {loading ? <div className="text-center py-16 text-purple-400">Loading...</div> : (
        <div className="space-y-3">
          {orders.map(o => (
            <div key={o.id} className="card p-0 overflow-hidden">
              <div
                className="flex items-center justify-between p-4 cursor-pointer hover:bg-purple-50/40 transition"
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${o.status === "PENDING" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
                    {o.status}
                  </span>
                  <span className="font-bold text-purple-700">₹{o.totalAmount?.toFixed(2)}</span>
                  ...
                  <div className="space-y-2">
                    {o.items?.map(i => (
                      <div key={i.id} className="flex justify-between text-sm">
                        ...
                        <span className="text-gray-500">x{i.quantity} — ₹{i.price?.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                </div>
